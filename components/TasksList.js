...
export default function TaskList({ tasks, onScroll }) {
  ...
  const isDatePassed = (dateString) => {
    ...
    if (date.isBefore(today, "day")) {
      return true; // Date has already passed
    } else {
      return false; // Date is today or in the future
    }
  };
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.taskContainer}>
            <View style={styles.imageContainer}>
              ...
              {/* <Text>{formatDate(item.FechaEnd)}</Text> */}
              <Text
